				...
				void OnGUI ()
				{
					...
					, "Text");
						...
						if (!isCollapsed) {
								...
								line = 5; //harcoded so player settings have fixed space
								...
								line++;
								GUI.color = controllableVars;
								showControllableSettings ();
								...
								if (useSeed) {
										...
										if (GUI.Button (generateButtonRect, new GUIContent ("apply", "Apply will generate a map according to the variables you have entered."), "Button1"))
												generate ();
								} else {
										...
										if (!isGenerating)
												...
												GUI.color = Color.red;
										if (GUI.Button (generateButtonRect, new GUIContent (isGenerating ? "stop" : "generate new", "Generate new will start generating random maps according to the variables you have entered."), "Button1"))
												isGenerating = !isGenerating; //the actual generate is called in the update
								}
								GUI.color = Color.white;
								...
								if (GUI.Button (saveButtonRect, new GUIContent ("save to file", "Save to file will save the last generated map as an .tmx file you can edit with the free Tiled software." + buildNotice), buttonStyle))
										saveToFile ();
								...
								if (GUI.Button (loadButtonRect, new GUIContent ("load from file", "Load from file will load a .tmx type map and materialize it." + buildNotice), buttonStyle))
										loadFromFile ();
...
//						DungeonPopulator.SetSpecificProperties (enemy_frequency, enemy_variety, boss_fights, trap_frequency, health_frequency, chest_frequency);
				}
...
//					ProDManager.Instance.SpawnEnemies();
		...
		#region generator specific Guis
				...
				private void showSettingsDungeon ()
				{
						...
						if (dungeon_room_Min_X % 2 == 0)
								dungeon_room_Min_X++;
						if (dungeon_room_Max_X % 2 == 0)
								dungeon_room_Max_X++;
						if (dungeon_room_Min_Y % 2 == 0)
								dungeon_room_Min_Y++;
						if (dungeon_room_Max_Y % 2 == 0)
								dungeon_room_Max_Y++;
						if (dungeon_room_Min_X > dungeon_room_Max_X)
								dungeon_room_Min_X = dungeon_room_Max_X;
						if (dungeon_room_Min_Y > dungeon_room_Max_Y)
								dungeon_room_Min_Y = dungeon_room_Max_Y;
				...
				private void showSettingsAlternativeDungeon ()
				{
						...
						if (alternativeDungeon_room_Min_X % 2 == 0)
								alternativeDungeon_room_Min_X++;
						if (alternativeDungeon_room_Max_X % 2 == 0)
								alternativeDungeon_room_Max_X++;
						if (alternativeDungeon_room_Min_Y % 2 == 0)
								alternativeDungeon_room_Min_Y++;
						if (alternativeDungeon_room_Max_Y % 2 == 0)
								alternativeDungeon_room_Max_Y++;
						if (alternativeDungeon_room_Min_X > alternativeDungeon_room_Max_X)
								alternativeDungeon_room_Min_X = alternativeDungeon_room_Max_X;
						if (alternativeDungeon_room_Min_Y > alternativeDungeon_room_Max_Y)
								alternativeDungeon_room_Min_Y = alternativeDungeon_room_Max_Y;
				...
				private void showSettingsDungeonRuins ()
				{
						...
						if (dungeonRuins_room_Min_X % 2 == 0)
								dungeonRuins_room_Min_X++;
						if (dungeonRuins_room_Max_X % 2 == 0)
								dungeonRuins_room_Max_X++;
						if (dungeonRuins_room_Min_Y % 2 == 0)
								dungeonRuins_room_Min_Y++;
						if (dungeonRuins_room_Max_Y % 2 == 0)
								dungeonRuins_room_Max_Y++;
						if (dungeonRuins_room_Min_X > dungeonRuins_room_Max_X)
								dungeonRuins_room_Min_X = dungeonRuins_room_Max_X;
						if (dungeonRuins_room_Min_Y > dungeonRuins_room_Max_Y)
								dungeonRuins_room_Min_Y = dungeonRuins_room_Max_Y;
				...
				private void showSettingsRoundRooms ()
				{
						...
						if (roundRooms_room_Diameter % 2 == 0)
								roundRooms_room_Diameter++;
				...
				private void showSettingsDwarfTown ()
				{
						...
						if (dwarfTown_room1_Min_X > dwarfTown_room1_Max_X)
								dwarfTown_room1_Min_X = dwarfTown_room1_Max_X;
						if (dwarfTown_room1_Min_Y > dwarfTown_room1_Max_Y)
								dwarfTown_room1_Min_Y = dwarfTown_room1_Max_Y;
						...
						if (dwarfTown_room2_Min_X > dwarfTown_room2_Max_X)
								dwarfTown_room2_Min_X = dwarfTown_room2_Max_X;
						if (dwarfTown_room2_Min_Y > dwarfTown_room2_Max_Y)
								dwarfTown_room2_Min_Y = dwarfTown_room2_Max_Y;
