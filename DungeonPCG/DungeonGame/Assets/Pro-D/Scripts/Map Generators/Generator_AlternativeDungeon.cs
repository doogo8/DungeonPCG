...
namespace ProD
{
	public static class Generator_AlternativeDungeon
	{
		...
		private static bool createDoors = false;
		...
		public static void SetSpecificProperties(string type_Abyss_, string type_Path_, string type_Wall_,
			...
			int repeat_, bool frameMap_, bool createDoors_)
		{
		...
		public static Map Generate()
		{
			...
			List<Room> rooms = MethodLibrary.CreateRooms(map, type_Wall, type_Path, room_Min_X, room_Max_X, room_Min_Y, room_Max_Y,
				room_Freq, room_Retry, 0);

			//MethodLibrary.AddNoiseOn_I(map, "PathWithGoblin", type_Path, 2);
			...
			if (createDoors)
				...
				{
					MethodLibrary.CreateDoors("Door", type_Path, map, room, false, -1);
				}


			MethodLibrary.PlaceStairs(map, 1, type_Path, "Entrance");
			MethodLibrary.PlaceStairs(map, 1, type_Path, "Exit", "Entrance", map.size_X * map_Size_Y / 30, walkableTypes);
